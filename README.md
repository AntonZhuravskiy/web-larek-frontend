...
**Назначение:** Управление данными заказа (способ оплаты, адрес доставки, контактная информация).
...
**Методы:**
- `setData()` - универсальный метод для обновления любого поля заказа с автоматической валидацией
- `validate()` - проверяет корректность всех данных заказа и эмитит событие с ошибками
- `validateDelivery()` - проверяет корректность данных доставки (способ оплаты и адрес)
- `validateContacts()` - проверяет корректность контактных данных (email и телефон)
- `getOrderData()` - возвращает данные заказа для объединения с данными корзины
...
- `isOpen()` - проверяет, открыто ли модальное окно в данный момент

#### 8. Класс `FormView<T>` (`view/FormView.ts`)
**Назначение:** Базовый класс для форм с автоматической обработкой событий ввода.

**Наследует:** `CommonView<IFormValidation>`

**Поля:**
- `inputList` - коллекция всех полей ввода в форме
...
- `set error()` - отображает пользователю сообщения об ошибках заполнения
- `updateErrors()` - обновляет отображение ошибок и управляет состоянием кнопки отправки
- `clear()` - очищает все поля формы и сбрасывает состояние
- `emitInput()` - эмитит события при изменении полей (автоматически для всех полей)
...
4. **Оформление заказа с валидацией:**
   - Пользователь открывает корзину (`AppEvents.BASKET_OPEN`)
   - При вводе данных в формах генерируются события `AppEvents.ORDER_UPDATE`
   - `OrderModel` получает данные через `setData()` и автоматически запускает валидацию
   - При ошибках валидации эмитится `AppEvents.ERRORS_UPDATE`
   - Presenter распределяет ошибки по соответствующим формам
   - Формы обновляют отображение ошибок и управляют состоянием кнопок
   - При корректных данных Presenter собирает полный заказ из `OrderModel` и `BasketModel`
   - Отправляет заказ через `ApiClient.createOrder()`
   - Показывает `SuccessView` с результатом

## Система валидации

Проект использует современную архитектуру валидации с автоматической обработкой ошибок:

### Принципы валидации
- **Валидация в модели:** Вся бизнес-логика проверки данных находится в `OrderModel`
- **Автоматическая валидация:** Каждое изменение данных запускает проверку
- **Реактивное обновление UI:** Формы автоматически обновляются при изменении ошибок
- **Разделение ошибок:** Ошибки доставки и контактов обрабатываются отдельно

### Поток валидации
1. **Ввод данных:** Пользователь изменяет поля в форме
2. **Эмиссия события:** `FormView` эмитит `AppEvents.ORDER_UPDATE` с данными поля
3. **Обновление модели:** `OrderModel.setData()` сохраняет данные и вызывает `validate()`
4. **Проверка данных:** Модель проверяет все поля и формирует объект ошибок
5. **Эмиссия ошибок:** `OrderModel` эмитит `AppEvents.ERRORS_UPDATE` с найденными ошибками
6. **Обновление UI:** Presenter распределяет ошибки по формам, формы обновляют отображение

### Ключевые события валидации
- `AppEvents.ORDER_UPDATE` - обновление данных заказа
- `AppEvents.ERRORS_UPDATE` - обновление ошибок валидации
